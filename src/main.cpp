...
Channel cg_accel_x =
    Channel(0x732, 60, 0, 1, 256.0, 0, "cg_accel_x", true, true);
Channel cg_accel_y =
    Channel(0x732, 60, 2, 3, 256.0, 0, "cg_accel_y", true, true);
Channel cg_accel_z =
    Channel(0x732, 60, 4, 5, 256.0, 0, "cg_accel_z", true, true);
...
void gpsPVTCallback(UBX_NAV_PVT_data_t *ubxDataStruct) {
  ...
  // Serial.println("lat: " + String(lat) + " lon: " + String(lon) + " alt: " +
  // String(alt) + " epoch: " + String(epoch));
}
...
void setupGPS() {
  ...
  gnssModule.setGPSL5HealthOverride(
      true); // Mark L5 signals as healthy - store in RAM and BBR

  gnssModule.setAutoPVTcallbackPtr(
      gpsPVTCallback); // Register the callback function

  gnssModule.softwareResetGNSSOnly(); // Restart the GNSS to apply the L5 health
                                      // override
...
void waitForGPSFix() {
  ...
  while ((gnssModule.getTimeValid() == false ||
         ...
         (millis() - startGPSInitTime <= 15000)) {
    gnssModule
        .checkUblox(); // Check for the arrival of new data and process it.
    gnssModule.checkCallbacks(); // Check if any callbacks are waiting to be
                                 // processed.
...
void loop() {
  ...
  gnssModule
      .checkCallbacks(); // Check if any callbacks are waiting to be processed.
  Can0.events();
}
